@if (empty($borda) || empty($borda['ranking']))
    <div
                    ...
                    <table class="w-full text-sm">
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach (collect($borda['ranking'])->sortBy('rank') as $altId => $data)
                                @php
                                    $isTop = $data['rank'] <= 3;
                                    $rowClass = match ($data['rank']) {
                                        1 => 'bg-amber-50/30 dark:bg-amber-900/10',
                                        default => 'hover:bg-slate-50 dark:hover:bg-slate-800/20',
                                    };
                                @endphp
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <section class="border-t-2 border-slate-900 dark:border-slate-800 pt-8">
                <h3
                    ...
                    Input Comparison Matrix (SMART Scores)
                </h3>
