    ...
    <div class="w-full space-y-6">
      ...
      <div class="flex flex-col justify-between gap-4 border-b-2 border-slate-100 px-2 pb-8 md:flex-row md:items-end">
        <div>
          ...
          <h1 class="text-3xl font-black uppercase tracking-tighter text-slate-800">Penugasan Decision Maker</h1>
        </div>
      ...
      <form method="POST" action="{{ route('decision-sessions.assign-dms.store', $decisionSession->id) }}"
          ...
          @forelse ($dms as $dm)
            <div @click="{{ $decisionSession->status === 'draft' ? "toggleDm($dm->id)" : '' }}"
              ...
              <div class="flex w-full items-start justify-between">
                <div
                  :class="selected.includes({{ $dm->id }}) ? 'bg-primary text-white shadow-primary/30' :
                ...
                <div class="flex h-6 w-6 items-center justify-center rounded-full border-2 transition-all"
                  :class="selected.includes({{ $dm->id }}) ? 'bg-primary border-primary text-white' : 'border-slate-200'">
              ...
              <div class="mt-2 min-w-0">
                <h3 :class="selected.includes({{ $dm->id }}) ? 'text-primary' : 'text-slate-800'"
              ...
              <div class="absolute bottom-0 left-0 h-1 transition-all"
                :class="selected.includes({{ $dm->id }}) ? 'w-full bg-primary' : 'w-0 bg-slate-200 group-hover:w-1/2'">
              </div>
            </div>
          @empty
            <div
              class="col-span-full flex flex-col items-center justify-center rounded-[3rem] border-4 border-dashed border-slate-100 py-32">
              <p class="text-xs font-black uppercase tracking-[0.4em] text-slate-300">Data Source Empty</p>
            </div>
          @endforelse
        </div>
        ...
        <div
          ...
          <div class="flex items-center gap-6 pl-4">
            <div>
              <p class="text-[9px] font-black uppercase tracking-widest text-slate-400">Selected Count</p>
              <p class="text-2xl font-black leading-none text-slate-800" x-text="selected.length"></p>
            </div>
