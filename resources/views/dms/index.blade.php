...
@section('content')
    ...
    <div class="pointer-events-none fixed right-6 top-6 z-[100] w-full max-w-sm space-y-3">
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show" x-transition
                class="pointer-events-auto flex items-center gap-4 rounded-2xl border border-emerald-100 bg-white p-4 shadow-2xl">
                ...
                <button @click="show = false" class="text-slate-300 transition-colors hover:text-slate-500">
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 6000)" x-show="show" x-transition
                class="pointer-events-auto flex items-center gap-4 rounded-2xl border border-rose-100 bg-white p-4 shadow-2xl">
    ...
    @if (($activeTab ?? null) === 'workspace' || empty($activeTab))
        <div class="mt-6 space-y-6 pb-10">
            {{-- Welcome Header --}}
            <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                <div class="relative z-10">
                    <p class="mb-1 text-xs font-black uppercase tracking-widest text-slate-400">
                        Sesi Keputusan Aktif
                    </p>

                    <h2 class="text-app mb-1 text-2xl font-black">
                    ...
                    <div class="mb-3 mt-2 flex flex-wrap items-center gap-3">
                        ...
                        <span
                            class="@if ($decisionSession->status === 'active') bg-blue-100 text-blue-700
                            ...
                            @else bg-slate-100 text-slate-600 @endif rounded-full px-3 py-1 text-xs font-bold">
                ...
                <div class="absolute right-0 top-0 -mr-4 -mt-4 h-32 w-32 rounded-full bg-slate-50 opacity-50"></div>
            </div>

            {{-- Info Cards Grid --}}
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div
                    class="hover:border-app group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all">
                    <div
                        class="text-app group-hover:bg-app mb-4 flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 transition-colors group-hover:text-white">
                ...
                <div
                    class="hover:border-app group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all">
                    <div
                        class="mb-4 flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600 transition-colors group-hover:bg-indigo-600 group-hover:text-white">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    class="hover:border-app group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all">
                    <div
                        class="mb-4 flex h-8 w-8 items-center justify-center rounded-lg bg-amber-50 text-amber-600 transition-colors group-hover:bg-amber-600 group-hover:text-white">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    class="hover:border-app group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all">
                    <div
                        class="mb-4 flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 transition-colors group-hover:bg-emerald-600 group-hover:text-white">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    {{-- WORKSPACE CONTENT --}}
    @if (($activeTab ?? null) === 'pairwise')
        @include('dms.pairwise.index')
    @endif

    @if (($activeTab ?? null) === 'group-weights')
        @include('dms.group-weights.index')
    @endif

    @if (($activeTab ?? null) === 'alternative-evaluations')
