  ...
  <div class="animate-in fade-in slide-in-from-bottom-4 space-y-8 pb-10 duration-700">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      @foreach ($cards as $card)
        <div class="adaptive-card hover:border-primary/50 group relative overflow-hidden p-6 transition-all duration-500">
          ...
          <div class="space-y-5">
            ...
            <div class="border-app/50 grid grid-cols-2 gap-4 border-y py-4">
              <div class="flex flex-col">
                ...
                <div class="mt-1">
                  ...
                  @endphp
                  @if ($status)
                    <span class="{{ $status['class'] }} rounded-md px-2 py-0.5 text-[10px] font-black uppercase">
                      {{ $status['label'] }}
                    </span>
                  @endif
                </div>
            ...
            <div class="flex items-center gap-2">
              @if ($session->dmHasCompleted)
                ...
                <p class="text-[11px] font-black uppercase tracking-tighter text-emerald-500">Penilaian
                  Kriteria Selesai</p>
              @else
                <div class="h-2 w-2 animate-pulse rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]">
                </div>
                <p class="text-[11px] font-black uppercase tracking-tighter text-amber-500">Perlu Isi
          ...
          <div class="mt-8">
            @if (in_array($session->status, $dmActivePhases))
              @php
                ...
                } elseif ($session->status === 'scoring' && !($session->dmEvaluationFinished ?? false)) {
