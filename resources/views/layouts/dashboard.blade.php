...
<body class="bg-app text-app min-h-screen font-sans antialiased">
  ...
  <div class="pointer-events-none fixed right-6 top-24 z-[100] flex w-full max-w-sm flex-col gap-3 sm:top-6">
    ...
    {{-- Toast Error --}}
    @if (session('error'))
      <div x-data="{ show: true, progress: 100 }" x-init="let interval = setInterval(() => { progress -= 1; if (progress <= 0) { show = false;
              clearInterval(interval); } }, 30);" x-show="show"
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-rose-600">Terjadi Kesalahan</h3>
  ...
  <script>
    document.addEventListener('alpine:init', () => {
      Alpine.data('scoringRule', (cfg) => ({
        // ... (Logika scoringRule Anda sudah benar, saya biarkan tetap di sini) ...
        open: true,
        isEdit: cfg.isEdit || false,
        inputType: cfg.inputType || '',
        preferenceType: cfg.preferenceType || 'linear',
        initialType: cfg.preferenceType || 'linear',
        min: Number(cfg.min || 0),
        max: Number(cfg.max || 0),
        semantics: cfg.semantics || {},
        utilities: cfg.utilities || {},
        init() {
          this.$watch('preferenceType', (v) => {
            if (v !== this.initialType) this.range().forEach(i => this.utilities[i] = this
        ...
        range() {
          let s = Number(this.min),
            e = Number(this.max);
          return s > e ? [] : Array.from({
            length: e - s + 1
          }, (_, i) => i + s);
        },
        calculateAutoValue(i) {
          let min = Number(this.min),
            max = Number(this.max),
            d = max - min;
          if (d <= 0) return (1.00).toFixed(2);
          let x = Math.max(0, Math.min(1, (i - min) / d)),
            v;
          if (this.preferenceType === 'concave') v = Math.sqrt(x);
          else if (this.preferenceType === 'convex') v = Math.pow(x, 2);
          else v = x;
          return v.toFixed(2);
        },
        utilityValue(i) {
          return (this.preferenceType === this.initialType && this.utilities[i] !== undefined) ? this.utilities[
            i] : this.calculateAutoValue(i);
        },
        syncUtilities() {
          if (this.preferenceType !== this.initialType) this.range().forEach(i => this.utilities[i] = this
            .calculateAutoValue(i));
        }
      }));
    });
