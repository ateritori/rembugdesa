    ...
    <div class="animate-in fade-in slide-in-from-bottom-4 w-full px-4 py-8 duration-700">
        ...
        <div class="w-full space-y-10">
                ...
                @foreach ($stats as $stat)
                    <div
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            <div class="min-w-0">
                                <p class="mb-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
                                    {{ $stat['label'] }}</p>
                                <p class="text-3xl font-black capitalize tracking-tighter text-slate-800">
                                    {{ $stat['value'] }}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                ...
                {{-- Card Pairwise --}}
                <div
                    class="group relative overflow-hidden rounded-[2.5rem] border-2 border-slate-100 bg-white p-10 shadow-sm">
                    <div
                        class="absolute right-0 top-0 p-8 opacity-5 transition-transform duration-700 group-hover:scale-110">
                    ...
                    <div class="relative z-10">
                        <div class="mb-8 flex items-end justify-between">
                            <div>
                                <h4 class="text-2xl font-black uppercase tracking-tight text-slate-800">Progres Pairwise
                                </h4>
                                <p class="mt-2 text-sm font-medium text-slate-400">Menghitung bobot kepentingan kriteria.
                                </p>
                            </div>
                            <span
                                class="text-4xl font-black tracking-tighter text-amber-500">{{ round($pairwisePercent) }}%</span>
                        </div>
                        <div class="h-5 w-full overflow-hidden rounded-full bg-slate-100 p-1 shadow-inner">
                            <div class="h-full rounded-full bg-gradient-to-r from-amber-400 to-amber-600 shadow-lg transition-all duration-1000"
                                style="width: {{ $pairwisePercent }}%"></div>
                        </div>
                        <div
                            class="mt-4 flex justify-between text-[11px] font-black uppercase tracking-widest text-slate-400">
                ...
                <div
                    ...
                    <div class="relative z-10">
                        <div class="mb-8 flex items-end justify-between">
                            <div>
                                <h4 class="text-2xl font-black uppercase tracking-tight text-slate-800">Penilaian Alternatif
                                </h4>
                                <p class="mt-2 text-sm font-medium text-slate-400">Status penilaian kandidat per kriteria.
                                </p>
                            </div>
                            <span
                                class="text-4xl font-black tracking-tighter text-emerald-500">{{ round($altPercent) }}%</span>
                        </div>
                        <div class="h-5 w-full overflow-hidden rounded-full bg-slate-100 p-1 shadow-inner">
                            <div class="h-full rounded-full bg-gradient-to-r from-emerald-400 to-emerald-600 shadow-lg transition-all duration-1000"
            ...
            <div class="w-full">
                ...
                @if ($decisionSession->status === 'draft')
                    @include('control.partials.action-card', [
                        ...
                        'title' => 'Aktifkan Sesi Sekarang',
                        'description' => 'Sistem akan membuka akses penilaian bagi Decision Maker.',
                        'left' => view('control.partials.status.draft-warning', get_defined_vars()),
                        'right' => view('control.partials.buttons.draft-activate', get_defined_vars()),
                    ])
                @endif
                ...
                @if ($decisionSession->status === 'configured')
                    @include('control.partials.action-card', [
                        ...
                        'phase' => 'Phase 02: Configured',
                        'title' => 'Sesi Siap Dinilai',
                        'description' => 'Konfigurasi selesai. Menunggu seluruh DM.',
                        'left' => view('control.partials.status.wait-pairwise', get_defined_vars()),
                        'right' => view('control.partials.buttons.start-alternative', get_defined_vars()),
                    ])
                @endif
                ...
                @if ($decisionSession->status === 'scoring')
                    @include('control.partials.action-card', [
                        ...
                        'phase' => 'Phase 03: Scoring',
                        'title' => 'Proses Penilaian Berlangsung',
                        'description' => 'Decision Maker sedang melakukan penilaian.',
                        'right' => view('control.partials.buttons.close-scoring', get_defined_vars()),
                    ])
                @endif
                ...
                @if ($decisionSession->status === 'aggregated')
                    @include('control.partials.action-card', [
                        ...
                        'phase' => 'Phase 04: Aggregated',
                        'title' => 'Hasil Teragregasi',
                        'description' => 'Sistem telah menyelesaikan perhitungan.',
                        'right' => view('control.partials.buttons.finalize', get_defined_vars()),
                    ])
                @endif
